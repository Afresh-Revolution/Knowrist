...
const App: React.FC = () => {
  // Authentication is always required - no environment variable check
  // Users must always be authenticated to access the dashboard
  ...
  const getAdminRoute = (): "main" | "super" | null => {
    ...
    if (path === "/mainadmin" || path.startsWith("/mainadmin/")) {
      console.log("Detected main admin route");
      return "main";
    }
    if (path === "/superadmin" || path.startsWith("/superadmin/")) {
      console.log("Detected super admin route");
      return "super";
    }
    return null;
  };

  const [adminRoute, setAdminRoute] = useState<"main" | "super" | null>(
    getAdminRoute
  );
  ...
  useEffect(() => {
    const handleRouteChange = () => {
      const newRoute = getAdminRoute();
      setAdminRoute(newRoute);
    };
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(() => {
    // Always check for existing authentication on initial load
  ...
  const [selectedDifficulty, setSelectedDifficulty] =
    useState<DailyDifficulty | null>(null); // Selected difficulty for activation code
  ...
  } | null>(null); // Selected pool for confirmation
  const [isPaymentSuccessOpen, setIsPaymentSuccessOpen] = useState(false); // Payment success modal visibility
  const [paymentResult, setPaymentResult] = useState<{
    success: boolean;
    activationCode?: string;
  } | null>(null); // Payment result
  const [isWordInputOpen, setIsWordInputOpen] = useState(false); // Word input modal visibility
  const [isSubmissionCompleteOpen, setIsSubmissionCompleteOpen] =
    useState(false); // Submission complete modal visibility
  const [submittedWords, setSubmittedWords] = useState<
    Array<{ correct: string; scrambled: string }>
  >([]); // Submitted words

  // Hover effect for profile modal
  const { dialogRef: profileModalRef, style: profileModalStyle } =
    useDialogHover();
  ...
  // Check for admin authentication first (before user authentication check)
  // This ensures admin login routes correctly to admin panel
  const adminToken = localStorage.getItem("admin_token");
  const storedAdminRole = localStorage.getItem("admin_role") as
    | "main"
    | "super"
    | null;
  const hasAdminAuth =
    adminToken &&
    storedAdminRole &&
    (storedAdminRole === "main" || storedAdminRole === "super");

  // If admin is authenticated, ALWAYS route to admin dashboard (never show game dashboard)
  if (hasAdminAuth) {
    const actualRole = storedAdminRole!;
    
    // If not on admin route, redirect to correct admin route
    if (!adminRoute || (adminRoute !== actualRole)) {
      const correctRoute = actualRole === "super" ? "/superadmin" : "/mainadmin";
      console.log("Admin authenticated - redirecting to:", correctRoute);
      window.location.href = correctRoute;
      return null; // Return null while redirecting
    }

    // Show admin panel if on correct admin route
    return <AdminPanel adminRole={actualRole} />;
  }

  // Show authentication form if user is not authenticated
  // Authentication is always required to access the dashboard
  // Only check this if not admin (admin is handled above)
  if (!isAuthenticated) {
    return (
      <div className="app">
  ...
  return (
    <div className="app">
      ...
      {showProfile && (
        <div
          className={`profile-overlay ${isProfileClosing ? "closing" : ""}`}
      ...
      <ActivationCode
        ...
        onContinue={() => {
          ...
          console.log(
            "Starting challenge with difficulty:",
            selectedDifficulty
          );
          setIsActivationCodeOpen(false);
          setSelectedDifficulty(null);
      ...
      <ConfirmEntry
        ...
        currentBalance={`₦${balance.toLocaleString("en-US", {
        ...
        onConfirm={() => {
          ...
          const entryFeeNum = parseFloat(
            selectedPool.entryAmount.replace(/[₦,]/g, "")
          );
          ...
          if (success) {
            ...
            const activationCode = `GAME-${Math.random()
              .toString(36)
              .substring(2, 7)
              .toUpperCase()}`;
      ...
      <SubmissionComplete
        ...
        wordAmounts={submittedWords.map((_, index) => {
          ...
          const difficultyMultiplier =
            selectedPool?.difficulty === "easy"
              ? 1
              : selectedPool?.difficulty === "medium"
              ? 1.5
              : 2;
          const wordLength = submittedWords[index]?.correct.length || 4;
          return Math.round(
            baseAmount * difficultyMultiplier * (wordLength / 4)
          );
        })}
        activationCode={paymentResult?.activationCode || ""}
